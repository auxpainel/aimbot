...
-- retorna o ponto de mira no personagem: cabeça se existir, senão tronco
local function GetAimPoint(character)
    ...
    elseif hrp and hrp:IsA("BasePart") then
        -- fallback: mira no tronco, com leve offset pra cima
        return hrp.Position + Vector3.new(0, 1.3, 0)
    end
    return nil
end
...
-- =======================
-- Substitua a função GetClosestTarget inteira por esta:
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character then
            if TeamCheck and player.Team == LocalPlayer.Team then
                -- pular aliado se opção de checar team ativada
                continue
            end
